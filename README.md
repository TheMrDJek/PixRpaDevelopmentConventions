...
**Пример обработки вложенного исключения:**
...
<details>
    <summary><b>Используйте finally для гарантированного освобождения ресурсов</b></summary>

Блок `finally` необходим для гарантированного освобождения ресурсов (файлы, соединения с БД, сетевые подключения) независимо от того, возникло исключение или нет.
...
public string ReadFileContent(string filePath)
{
    ...
    {
        Logger.Error($"Ошибка чтения файла {filePath}: {ex.Message}");
        // Закрываем ресурсы только в случае ошибки
        reader.Close();
        fileStream.Close();
        throw;
    }
    
    // ❌ Проблема: если исключение не возникнет, 
    // reader и fileStream никогда не будут закрыты
    reader.Close();
    fileStream.Close();
}
```
...
public string ReadFileContent(string filePath)
{
    ...
    {
        // Гарантированно закрываем ресурсы в любом случае
        reader.Close();
        fileStream.Close();
    }
}
...
public string ReadFileContent(string filePath)
{
    // using автоматически вызовет Dispose() независимо от исключений
    using (var fileStream = new FileStream(filePath, FileMode.Open))
    using (var reader = new StreamReader(fileStream))
    {
        return reader.ReadToEnd();
    }
    // Ресурсы будут освобождены даже при возникновении исключения
}
```

**Типичные ресурсы, требующие правильного освобождения:**
- Файловые потоки и дескрипторы
- Соединения с базами данных
- Сетевые соединения
- Блокировки и семафоры
- Объекты COM и неуправляемые ресурсы

**Важно:** В PIX RPA при работе с файлами и базами данных рекомендуется всегда использовать конструкцию `using` или блок `finally` для гарантированного освобождения ресурсов, чтобы избежать утечек памяти и блокировок файлов.
...
❌ **Неправильно:**
```csharp
// Низкоуровневый метод подавляет исключение
public bool SaveRecord(DataRecord record)
{
    try
    {
    ...
    {
        // Подавление исключения и возврат false
        return false;
    }
}

// Вызывающий код не имеет информации о причине ошибки
public void ProcessOrder(Order order)
{
    if (!SaveRecord(new DataRecord(order)))
    {
        // Нет информации о причине ошибки
        // Что делать дальше? Повторить? Отменить? Уведомить пользователя?
...
✅ **Правильно:**
```csharp
// Низкоуровневый метод пробрасывает исключение с контекстом
public void SaveRecord(DataRecord record)
{
    ...
    {
        // Добавляем контекст и пробрасываем исключение дальше
        throw new DataAccessException($"Ошибка сохранения записи ID={record.Id}", ex);
    }
}

// Высокоуровневый метод принимает решение на основе типа исключения
public void ProcessOrder(Order order)
{
    try
    {
        SaveRecord(new DataRecord(order));
        CompleteOrderProcessing(order);
    }
    catch (DataAccessException ex) when (IsDatabaseUnavailable(ex))
    {
        // Решение: временная недоступность - поставить в очередь для повторной обработки
        QueueForRetry(order);
        Logger.Warning($"Заказ {order.Id} поставлен в очередь повторной обработки: {ex.Message}");
    }
    catch (DataAccessException ex) when (IsDataConstraintViolation(ex))
    {
        // Решение: проблема с данными - отправить на ручную обработку
        SendToManualProcessing(order, ex.Message);
        Logger.Error($"Заказ {order.Id} отправлен на ручную обработку: {ex.Message}");
    }
    catch (Exception ex)
    {
        // Общая обработка других исключений
        Logger.Error($"Непредвиденная ошибка при обработке заказа {order.Id}: {ex.Message}");
        NotifyAdministrator(order, ex);
        throw; // Проброс исключения дальше, если не смогли обработать
    }
}
```
